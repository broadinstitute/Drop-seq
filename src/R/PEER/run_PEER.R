...
# Check for no arguments, and print help if there are none, then exit
...
if (has.cov) {
    cols <- c(colnames(covar.df), sapply(seq_len(ncol(X)-dim(covar.df)[2]),function(x){paste0("InferredCov",x)}))
} else {
